...
function AdminUsers({ token }) {
  ...
  const getAvatarStyle = (username, role) => {
    if (role === 'ADMIN') {
      return 'bg-gradient-to-tr from-zinc-900 to-zinc-700 dark:from-zinc-105 dark:to-zinc-300 text-white dark:text-black';
    }
    const colors = [
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-surface/80 backdrop-blur border border-border rounded-2xl p-6 shadow-sm">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Beheer actieve gasten en admin accounts. Totaal: <strong className="text-zinc-900 dark:text-zinc-105">{totalCount}</strong> | Gasten: <strong className="text-zinc-900 dark:text-zinc-105">{guestCount}</strong> | Beheerders: <strong className="text-zinc-900 dark:text-zinc-105">{adminCount}</strong> | 18+ Geverifieerd: <strong className="text-red-600 dark:text-red-400">{verifiedCount}</strong>
          </p>
        </div>
          ...
          <div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-0.5 rounded-xl self-end sm:self-auto shadow-sm">
            <button
              ...
              className={`p-2 rounded-lg transition-all duration-200 ${
                viewMode === 'grid' 
                  ? 'bg-white dark:bg-zinc-805 text-zinc-900 dark:text-white shadow-sm' 
      ...
      {filteredUsers.length === 0 ? (
        <div className="bg-surface border border-dashed border-zinc-300 dark:border-zinc-850 rounded-2xl p-12 text-center shadow-sm">
          ...
          <h3 className="font-bold text-zinc-900 dark:text-zinc-100">Geen accounts gevonden</h3>
          <p className="text-xs text-zinc-400 dark:text-zinc-555 mt-1.5">
            ...
            <div key={u.id} className="group bg-surface border border-border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between overflow-hidden">
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col items-end gap-1.5">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${
                      u.role === 'ADMIN'
                        ? 'bg-zinc-950 text-zinc-50 dark:bg-white dark:text-zinc-950 border border-zinc-900 dark:border-white shadow-sm'
                        : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-900/30 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800'
                    }`}>
                      {u.role === 'ADMIN' ? 'Beheerder' : 'Gast'}
                    </span>
                    {u.is_18_plus && (
                      <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-extrabold bg-red-100 text-red-750 dark:bg-red-950/30 dark:text-red-400 border border-red-200 dark:border-red-900/30 shadow-sm animate-pulse-border">
                        🔞 18+
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-1">
                  <h3 className="font-bold text-base text-zinc-900 dark:text-zinc-50 truncate" title={u.username}>
                    {u.username}
                  </h3>
                  <div className="text-[10px] text-zinc-400 dark:text-zinc-500 flex items-center gap-1.5">
                    ...
                    <span>Laatste login: <strong className="font-semibold text-zinc-650 dark:text-zinc-350">{u.last_login ? new Date(u.last_login).toLocaleString() : 'Nooit'}</strong></span>
                  </div>
                </div>
              </div>

              {/* Action Buttons */}
              <div className="pt-4 mt-5 border-t border-zinc-50 dark:border-zinc-900 flex gap-2">
                <button
                  onClick={() => handleEdit(u)}
                  className="flex-1 text-center font-semibold text-xs text-zinc-700 dark:text-zinc-300 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-850 py-2.5 rounded-xl transition-all shadow-sm"
                >
                  Bewerken
                </button>
                {u.id !== currentUserId ? (
                  <button
                    ...
                    className="flex-1 text-center font-bold text-xs text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-305 bg-red-50 hover:bg-red-100/70 dark:bg-red-950/20 dark:hover:bg-red-950/40 py-2.5 rounded-xl border border-transparent hover:border-red-200 dark:hover:border-red-900/30 transition-all"
                  >
                    Verwijderen
                  </button>
                ) : (
                  <div className="flex-1 flex items-center justify-center text-xs text-zinc-400 dark:text-zinc-600 italic py-2.5 font-medium">
                    Jij
                  </div>
                )}
              </div>
                  ...
                  <tr key={u.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="font-semibold text-zinc-900 dark:text-zinc-105 leading-snug">
                          {u.username}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        u.role === 'ADMIN' 
                          ? 'bg-zinc-950 text-zinc-50 dark:bg-white dark:text-zinc-950 border border-zinc-900 dark:border-white shadow-sm' 
                          : 'bg-zinc-100 text-zinc-700 dark:bg-zinc-900/30 dark:text-zinc-350 border border-zinc-200 dark:border-zinc-800'
                      }`}>
                        {u.role === 'ADMIN' ? 'Beheerder' : 'Gast'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {u.is_18_plus ? (
                        <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-bold bg-red-100 text-red-750 dark:bg-red-950/30 dark:text-red-400 border border-red-200 dark:border-red-900/30 shadow-sm">
                          🔞 18+ Geverifieerd
                        </span>
                      ) : (
                        <span className="text-xs text-zinc-400 dark:text-zinc-555 font-medium">Niet geverifieerd</span>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                      ...
                      {u.id !== currentUserId ? (
                        <button
                          onClick={() => handleDeleteUser(u.id, u.username)}
                          className="text-xs font-semibold text-red-655 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 px-3 py-1.5 rounded-lg border border-transparent hover:border-red-200 dark:hover:border-red-900/50 hover:bg-red-50/50 dark:hover:bg-red-950/20 transition-all duration-200"
                        >
                          Verwijderen
                        </button>
                      ) : (
          ...
          <div className={`fixed right-0 top-0 bottom-0 w-full max-w-md bg-surface z-50 shadow-2xl flex flex-col justify-between transition-transform duration-300 ease-out transform ${
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <form id="drawer-user-form" onSubmit={handleSubmit} className="space-y-6">
                ...
                {error && (
                  <div className="text-red-500 bg-red-50 dark:bg-red-950/20 dark:text-red-405 p-3.5 rounded-xl text-xs border border-red-100 dark:border-red-900/30 animate-pulse-border">
                    {error}
                  </div>
                )}
                ...
                {/* Section 1: Algemeen */}
                <div className="space-y-4">
                  <span className="text-[10px] font-extrabold text-zinc-405 dark:text-zinc-500 uppercase tracking-wider block border-b border-zinc-100 dark:border-zinc-900 pb-1">Basisgegevens</span>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      type="text" 
                      placeholder="Bijv. Tafel 12 of Medewerker_Lisa"
                      value={formData.username} 
                      onChange={e => setFormData({...formData, username: e.target.value})} 
                      className="w-full px-3.5 py-2.5 bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:bg-white dark:focus:bg-zinc-900 transition-all text-xs focus:ring-1 focus:ring-black dark:focus:ring-white"
                      required 
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      type="password" 
                      placeholder={editingUserId ? "Nieuw wachtwoord (optioneel)" : "Wachtwoord invoeren"}
                ...
                <div className="space-y-4">
                  <span className="text-[10px] font-extrabold text-zinc-405 dark:text-zinc-500 uppercase tracking-wider block border-b border-zinc-100 dark:border-zinc-900 pb-1">Type Account & Bevoegdheden</span>
                  
                  {/* Premium Segmented Role selector */}
                  <div className="space-y-2">
                    <label className="block text-xs font-semibold text-zinc-705 dark:text-zinc-400">
                      Rol / Toegangsniveau
                    </label>
                  ...
                  {/* 18+ Checkbox wrapper */}
                  <div className="flex items-center gap-3 py-3.5 px-4 bg-zinc-50 dark:bg-zinc-900/30 rounded-xl border border-border mt-3">
                    <input 
                      type="checkbox" 
                      id="is_18_plus"
