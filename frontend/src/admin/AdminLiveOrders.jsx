...
function AdminLiveOrders({ token }) {
 ...
 soundEnabled ? 'bg-primary text-primary-text border-transparent'
 ...
 {grouped.length === 0 ? (
 <div className="bg-surface border border-dashed border-zinc-300 rounded-3xl p-12 text-center">
 ...
 <h3 className="font-bold text-primary">Helemaal Bijgewerkt!</h3>
 ...
 className={`bg-surface border hover:border-black rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-200 cursor-grab active:cursor-grabbing flex flex-col justify-between ${
 isPreparing ? 'border-warning-border shadow-amber-50/10 ' : 'border-border'
 }`}
 >
 <div>
 {/* Card Header */}
 ...
 <div>
 <span className="text-xs font-bold uppercase tracking-wider text-muted">Klant</span>
 <h3 className="text-lg font-bold text-primary leading-tight flex items-center gap-1.5 flex-wrap">
 {group.guestName}
 {isPreparing && (
 <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-warning-bg text-warning border border-warning-border animate-pulse">
 Bereiden...
 </span>
 )}
 </h3>
 ...
 <span className="text-[10px] font-semibold text-muted uppercase">Wachttijd</span>
 <span className="text-sm font-mono font-bold text-primary tracking-wide">
 ...
 <div key={order.id} className="border-t border-border-subtle pt-2.5">
 ...
 {item.is_18_plus && (
 <span className="bg-red-50 text-red-650 text-[9px] font-bold px-1.5 py-0.5 rounded border border-danger-border">
 18+
 </span>
 )}
 </div>
 ...
 <div className="text-xs text-secondary pl-2 border-l border-border">
 {Object.entries(item.selected_options).map(([k, v]) => (
 <div key={k}>{k}: <span className="font-semibold text-secondary">{v}</span></div>
 ))}
 </div>
 )}
 ...
 {/* Remarks */}
 {item.remark && (
 <div className="text-xs bg-warning-bg text-warning p-2 rounded-lg border border-warning-border italic">
 ...
 className="flex-1 bg-amber-500 hover:bg-amber-600 text-white py-2.5 rounded-xl text-xs font-bold transition-all text-center"
 >
 Start Bereiden
 </button>
 )}
 ...
 isPreparing ? 'bg-primary text-primary-text hover:bg-surface-hover'
 : 'bg-zinc-100 text-zinc-705 hover:bg-surface-hover '
 }`}
 >
 Afronden
 </button>
